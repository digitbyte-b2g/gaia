...
MediaPlaybackWidget.prototype = {
  ...
  updateNowPlaying: function mpw_updateNowPlaying(metadata) {
    ...
    if (picture) {
      var imageBlob = window.URL.createObjectURL(picture);
      console.log(imageBlob);
      this.container.style.background = `var(--background-plus) url(${imageBlob}) no-repeat left top / 10rem`;

      // We preload the image so we don't get back for a color.
      var image = new Image();
      image.src = imageBlob;
      image.onload = () => {
        colorPicker(imageBlob, { colors: 2 }).then((colors) => {
          this.container.style.setProperty('--background-plus', colors[0]);

          switch (lightOrDark(colors[0])) {
            case 'light':
              colorPicker(imageBlob, { colors: 2, brightness: 0.25 }).then((colors1) => {
                this.container.style.setProperty('--text-color', colors1[1]);
              });
              break;

            case 'dark':
              colorPicker(imageBlob, { colors: 2, brightness: 1.75 }).then((colors1) => {
                this.container.style.setProperty('--text-color', colors1[1]);
              });
              break;
          }
