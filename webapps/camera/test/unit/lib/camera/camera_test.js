...
suite('lib/camera/camera', function() {
  ...
  setup(function() {
    ...
    if (!navigator.b2g.cameras) { navigator.b2g.cameras = mozCameras; }
    if (!navigator.b2g.getDeviceStorage) { navigator.b2g.getDeviceStorage = function() {}; }

    this.sandbox = sinon.sandbox.create();
    this.sandbox.stub(navigator, 'getDeviceStorage').returns(this.videoStorage);
    this.sandbox.stub(navigator.b2g.cameras);

    navigator.b2g.cameras.getListOfCameras.returns([]);
  ...
  suite('Camera#requestCamera()', function() {
    setup(function() {
      ...
      navigator.b2g.cameras.getCamera.returns({
    ...
    test('Should call `navigator.b2g.cameras.getCamera()` with currently selected camera', function() {
      this.camera.requestCamera('back');
      assert.isTrue(navigator.b2g.cameras.getCamera.calledWith('back'));
      navigator.b2g.cameras.getCamera.reset();

      this.camera.requestCamera('front');
      assert.isTrue(navigator.b2g.cameras.getCamera.calledWith('front'));
    });
    ...
    test('Should call get camera with the passed config', function() {
      ...
      assert.isTrue(navigator.b2g.cameras.getCamera.calledWith('back', this.mozCameraConfig));
    });
    ...
    test('It attempts to re-request the camera if \'NS_ERROR_NOT_INITIALIZED\'', function() {
      ...
      // First attempt
      this.camera.requestCamera();
      sinon.assert.called(navigator.b2g.cameras.getCamera);
      navigator.b2g.cameras.getCamera.reset();
      ...
      sinon.assert.notCalled(navigator.b2g.cameras.getCamera);
    });
  });
