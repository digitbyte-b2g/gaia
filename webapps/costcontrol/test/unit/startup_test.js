...
if (!window.navigator.b2g.mobileConnections) {
  window.navigator.b2g.mobileConnections = null;
}
...
suite('Application Startup Modes Test Suite >', function() {
  ...
  suiteSetup(function() {
    ...
    realMozMobileConnections = window.navigator.b2g.mobileConnections;
    window.navigator.b2g.mobileConnections = MockNavigatorMozMobileConnections;
  ...
  suiteTeardown(function() {
    ...
    window.navigator.b2g.mobileConnections = realMozMobileConnections;
  ...
  suite('supportCustomizeMode setting', function() {
    ...
    suiteSetup(function() {
      realMozAlarms = window.navigator.b2g.alarmManager;
      window.navigator.b2g.alarmManager = MockMozAlarms;
    });

    suiteTeardown(function() {
      window.navigator.b2g.alarmManager = realMozAlarms;
    });
    ...
    test('Start up with custom mode when functionality is disabled remove ' +
      ...
      function(done) {
        ...
        this.sinon.stub(navigator.b2g.alarmManager, 'remove', function() {});
        setupCardState({cardState: 'ready'});
        window.ConfigManager = new MockConfigManager(
          {
            fakeSettings: {
        ...
        window.addEventListener('ftestarted', function _onftestarted(evt) {
          ...
          assert.equal(window.ConfigManager.option('trackingPeriod'),
                       expectedMode);
          sinon.assert.calledWith(navigator.b2g.alarmManager.remove, nextResetAlarmId);
