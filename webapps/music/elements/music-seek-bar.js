...
`<style>
  ...
  #seek-bar-progress::-moz-progress-bar {
    background-color: var(--music-text-color, var(--text-color, #333));
    border-radius: 1.2rem;
    height: 2.4rem;
  }
  #seek-bar-indicator {
  ...
  #seek-bar-indicator:after {
    content: '';
    background-color: var(--music-accent-color, var(--background-plus, #fff));
...
Object.defineProperty(proto, 'remainingTime', {
  ...
  set: function(value) {
    ...
    window.requestAnimationFrame(() => {
      ...
      this.els.seekBarProgress.max = this.duration;
      this.els.seekBarProgress.value = this._elapsedTime;
