...
PlayerView.prototype.update = function() {
  this.getPlaybackStatus().then((status) => {
    ...
    this.getSongArtwork(status.filePath)
      .then((url) => {
        var themeColor = window.top.document.querySelector('meta[name="theme-color"]');
        this.artwork.src = url;

        setTimeout(function() {
          colorPicker(url, { linearGradient: 'to bottom', colors: 2 }).then(colors => {
            document.body.style.setProperty('--music-accent-color', colors);
          });
          colorPicker(url, { colors: 2 }).then(colors => {
            window.top.document.body.style.setProperty('--header-background', colors[0]);
            if (themeColor) {
              themeColor.setAttribute('content', colors[0]);
            }
          });

          if (lightOrDark(colorPicker(url))) {
            colorPicker(url, { colors: 2, brightness: 0.25 }).then(colors => {
              document.body.style.setProperty('--music-text-color', colors[1]);
              window.top.document.body.style.setProperty('--header-color', colors[1]);
              window.top.document.body.style.setProperty('--header-icon-color', colors[1]);
              window.top.document.body.style.setProperty('--header-button-color', colors[1]);
            });
          } else {
            colorPicker(url, { colors: 2, brightness: 1.75 }).then(colors => {
              document.body.style.setProperty('--music-text-color', colors[1]);
              window.top.document.body.style.setProperty('--header-color', colors[1]);
              window.top.document.body.style.setProperty('--header-icon-color', colors[1]);
              window.top.document.body.style.setProperty('--header-button-color', colors[1]);
