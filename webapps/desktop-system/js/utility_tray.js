...
exports.UtilityTray = {
  ...
  recalculateNotificationsContainerHeight() {
    this.notificationsContainer.style.maxHeight = (this.motionElement.offsetHeight -
      (this.footerContainer.offsetHeight + 197)) + 'px';
  ...
  init: function ut_init() {
    ...
    // Brightness slider
    var brightnessSlider = this.brightnessSlider.querySelector('.slider');
    var brightnessAutoCheckbox = this.brightnessSlider.querySelector('.auto');

    brightnessSlider.min = 0.1;
    brightnessSlider.max = 1;
    brightnessSlider.step = 0.01;

    var screenBrightness = navigator.mozSettings.createLock().get('screen.brightness');
    screenBrightness.onsuccess = function() {
      brightnessSlider.value = screenBrightness.result['screen.brightness'];
    };

    var screenAutomaticBrightness = navigator.mozSettings.createLock().get('screen.automatic-brightness');
    screenAutomaticBrightness.onsuccess = function() {
      brightnessAutoCheckbox.checked = screenAutomaticBrightness.result['screen.automatic-brightness'];
    };

    brightnessSlider.onchange = function() {
      navigator.mozSettings.createLock().set({ 'screen.brightness': brightnessSlider.value });
    };

    brightnessAutoCheckbox.onchange = function() {
      navigator.mozSettings.createLock().set({ 'screen.automatic-brightness': brightnessAutoCheckbox.checked });
    };

    // Audio slider
    var audioSlider = this.audioSlider.querySelector('.slider');
    audioSlider.min = 0;
    audioSlider.max = 15;

    var audioVolume = navigator.mozSettings.createLock().get('audio.volume.content');
    audioVolume.onsuccess = function() {
      audioSlider.value = audioVolume.result['audio.volume.content'];
    };

    audioSlider.onchange = function() {
      navigator.mozSettings.createLock().set({ 'audio.volume.content': audioSlider.value });
      new Audio('/shared/resources/media/notifications/notifier_wos.wav').play();
    };
    ...
    this.statusbar.addEventListener('click', this);
  ...
  handleEvent: function ut_handleEvent(evt) {
    ...
    switch (evt.type) {
      case 'click':
        ...
        var width = this.motionElement.offsetWidth;
        if (this.isActive()) {
          this.overlay.classList.add('transition-opening');
          this.overlay.addEventListener('animationend', () => {
            this.overlay.classList.remove('transition-opening');
            this.motionElement.scrollLeft = rtl ? -width : width;
          });
        } else {
          this.overlay.classList.add('transition-closing');
          this.overlay.addEventListener('animationend', () => {
            this.overlay.classList.remove('transition-closing');
