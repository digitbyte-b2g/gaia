 ...
 * and scroll events, but `scrollLeft` is always an out-of-date snapshot.
 ...
 * element's scrollLeft whenever a touch or scroll event occurs. In this way,
...
function UtilityTrayMotion(el) {
  ...
  // Begin closed. Note that scrollLeft may be the inverse of what you'd expect;
  // a scrollLeft of zero indicates the tray is open; a scrollLeft of scrollLeftMax
  // indicates the tray is closed.
  this._setState('closing');
  this.position = this.el.scrollLeft = this.el.scrollLeftMax;
...
UtilityTrayMotion.prototype = {
  ...
  _setState(newState) {
    ...
    if (oldState !== newState) {
      ...
      this.statusbar.classList.toggle('tray-open', newState !== 'opening');
  ...
  /**
   * The desired scroll offset (i.e. between 0 and this.el.scrollLeftMax),
   ...
   * is equal to scrollLeftMax, the tray is hidden.
   *
  ...
  markPosition(source) {
    ...
    this.maxPosition = this.el.scrollLeftMax;
