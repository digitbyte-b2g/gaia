...
var NotificationScreen = {
  ...
  start: function() {
    ...
    SettingsListener.observe('notification.ringtone', '', function(value) {
      LazyLoader.load(['http://shared.localhost:8081/js/settings_url.js']).then(function() {
        if (!self.ringtoneURL) {
  ...
  resendStoredNotifications: function() {
    if ('mozSettings' in navigator && SettingsObserver) {
      var key = 'notifications.resend';
      var req = SettingsObserver.getValue(key);
