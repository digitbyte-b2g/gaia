...
(function(exports) {
  ...
  WallpaperManager.prototype = {
    ...
    _checkSize: function(blob, needsToBeSaved) {
      ...
      LazyLoader.load('http://shared.localhost:8081/js/image_utils.js', function() {
        ImageUtils
    ...
    _publish: function(blob) {
      ...
      document.getElementById('screen').style.backgroundImage =
        ...
        'url(' + this._blobURL + ')';
      document.getElementById('screen-background').style.backgroundImage =
        'linear-gradient(rgba(0, 0, 0, 0.1), rgba(0, 0, 0, 0.1)),' +
        'url(' + this._blobURL + ')';
      document.getElementById('screen-backdrop').style.backgroundImage =
        '-moz-element(#windows), linear-gradient(rgba(0, 0, 0, 0.1), rgba(0, 0, 0, 0.1)),' +
        'url(' + this._blobURL + ')';
