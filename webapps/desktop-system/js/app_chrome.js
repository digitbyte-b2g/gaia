...
(function(exports) {
  ...
  AppChrome.prototype._registerEvents = function ac__registerEvents() {
    if (this.useCombinedChrome()) {
      LazyLoader.load('http://shared.localhost:8081/js/bookmarks_database.js').then(() => {
        this.updateAddToHomeButton();
      }).catch((err) => {
        console.error(err);
      });
      LazyLoader.load('http://shared.localhost:8081/elements/gaia_overflow_menu/script.js');
  ...
  AppChrome.prototype.openNewTab = function ac_openNewTab({ url, isPrivate, app } = { url: '', isPrivate: false, app: null }) {
    ...
    if (app) {
      ...
      browser = document.createElement('web-view');
      ...
      if (url) {
        ...
        browser.setAttribute('origin', 'https://www.google.com');
