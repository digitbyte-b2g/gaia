...
PROFILE_WEBAPPS_DIR?=$(PROFILE_FOLDER)$(SEP)webapps
export PROFILE_WEBAPPS_DIR
...
SYSTEM?=$(SCHEME)system.$(GAIA_DOMAIN)/index.html
...
{ \
  ...
  "INSTALLED_DIR" : "$(PROFILE_WEBAPPS_DIR)$(SEP)installed", \
}
endef
...
# Erase all the indexedDB databases on the device, so apps have to rebuild them.
delete-databases:
	@echo 'Stopping b2g'
	@$(ADB) shell stop b2g
...
# on your device, and you have adb in your path, then you can use the
...
# If we've got old style directories on the device, rename them first.
...
# remove the memories and apps on the device
purge:
	$(ADB) shell stop b2g
...
# push $(PROFILE_FOLDER)/settings.json and $(PROFILE_FOLDER)/contacts.json (if CONTACTS_PATH defined) to the device
