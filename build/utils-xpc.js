...
function getWebapp(app, config) {
  ...
  manifestFile.append('manifest.webmanifest');

  let updateFile = appDir.clone();
  updateFile.append('update.webmanifest');
  ...
  if (metaData.exists()) {
    ...
    zip.load(joinPath(webapp.sourceDirectoryFilePath, 'application.zip'),
             'read');
    webapp.pckManifest = JSON.parse(zip.file('manifest.webmanifest'));
  ...
  webapp.buildManifestFilePath = joinPath(webapp.buildDirectoryFilePath,
    'manifest.webmanifest'); // buildManifestFile
...
function getApp(parent, appname, gaiadir, distdir) {
  ...
  var filename;
  if (getFile(app.path, 'manifest.webmanifest').exists()) {
