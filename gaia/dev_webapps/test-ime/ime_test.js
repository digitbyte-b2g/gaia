...
suite('Input Method', function() {
  ...
  function sendKeys(chars, expected, next) {
    var completed = 0;
    var icc = navigator.b2g.inputMethod.inputcontext;
    if (!icc)
      return;
    chars.forEach(function(c) {
      ...
      sk.then(function() {
        if (++completed === chars.length) {
          navigator.b2g.inputMethod.inputcontext.getText().then(function(v) {
  ...
  function onIcc(needIc, callback) {
    navigator.b2g.inputMethod.addEventListener('inputcontextchange', function onicc() {
      if (!navigator.b2g.inputMethod.inputcontext && needIc) {
        return;
      }
      navigator.b2g.inputMethod.removeEventListener('inputcontextchange', onicc);
      callback(navigator.b2g.inputMethod.inputcontext);
    });
  }
  ...
  setup(function(next) {
    ...
    if (!navigator.b2g.inputMethod.inputcontext) {
      return next();
    }
