...
suite('system/LockScreenStateManager', function() {
  ...
  setup(function() {
      ...
      this.createLock = () => {
        return (function() {
          this.get = (key) => {
            ...
            SettingsObserver._deferredReadings[key] = new Deferred();
            SettingsObserver._deferredReadings[key].promise =
              SettingsObserver._deferredReadings[key].promise
              .then((fetchingResult) => {
                lock.result = {};
    ...
    SettingsObserver = new mockMozSettings();
    subject = (new LockScreenStateManager())
      .start(new mockLockScreen());
    SettingsObserver._reset();
  ...
  suite('"integration tests"', function() {
    ...
    test('if we turn on the debug setting, it will create '+
      ...
      SettingsObserver._deferredReadings['debug.gaia.enabled'].promise =
      SettingsObserver
    ...
    test('if we turn off the debug setting, it will create '+
      ...
      var observerPromise = SettingsObserver
        ._deferredObservers['debug.gaia.enabled'].promise;
      SettingsObserver._deferredObservers['debug.gaia.enabled']
