...
(function(exports) {
  ...
  SoundManager.prototype.start = function sm_start() {
    LazyLoader.load(['http://shared.localhost:8081/js/async_semaphore.js']).then(() => {
  ...
  SoundManager.prototype.handleEvent = function sm_handleEvent(e) {
    switch (e.type) {
      case 'volumeup':
        ...
        new Audio('resources/sounds/volume_up.wav').play();
        break;
      case 'volumedown':
        this.handleVolumeKey(-1);
        new Audio('resources/sounds/volume_down.wav').play();
        break;
      case 'mute':
        this.setMute(true);
        break;
