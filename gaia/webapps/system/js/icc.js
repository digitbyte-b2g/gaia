...
var icc = {
  ...
  init: function icc_init() {
    ...
    var reqDisplayTimeout = window.SettingsObserver.getValue(
    ...
    window.SettingsObserver.addObserver('icc.displayTextTimeout',
      function(e) {
        self._displayTextTimeout = e.settingValue;
      }
    );
    // Update inputTimeout with settings parameter
    var reqInputTimeout = window.SettingsObserver.getValue(
    ...
    window.SettingsObserver.addObserver('icc.inputTextTimeout',
      function(e) {
        self._inputTimeout = e.settingValue;
      }
    );
    // Update toneDefaultTimeout with settings parameter
    var reqToneDefaultTimeout = window.SettingsObserver.getValue(
    ...
    window.SettingsObserver.addObserver('icc.toneDefaultTimeout',
      function(e) {
  ...
  getConnection: function icc_getConnection(iccId) {
    ...
    for (var i = 0; i < window.navigator.b2g.mobileConnections.length; i++) {
      if (window.navigator.b2g.mobileConnections[i].iccId === iccId) {
        DUMP('ICC Connection ' + i + ' found for ' + iccId);
        return window.navigator.b2g.mobileConnections[i];
      }
    }
    return null;
  },
  ...
  clearMenuCache: function icc_clearMenuCache(callback) {
    ...
    var resetApplications = window.SettingsObserver.setValue({
      'icc.applications': '{}'
    });
