...
suite('start testing > ', function() {
  ...
  setup(function(done) {
    ...
    requireCtx(modules, function(usb_transfer) {
      realGetDeviceStorages = window.navigator.b2g.getDeviceStorages;
      window.navigator.b2g.getDeviceStorages = sinon.stub().returns([{
        canBeShared: true
      }]);
  ...
  teardown(function() {
    window.navigator.b2g.getDeviceStorages = realGetDeviceStorages;
  });
