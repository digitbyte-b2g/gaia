...
(function(exports) {
  ...
  WallpaperManager.prototype = {
    ...
    start: function() {
      ...
      query.onsuccess = function() {
        ...
        if (wallpaper) {
          ...
          image.onload = () => {
            colorPicker(wallpaper, { colors: 2 }).then((colors) => {
              lockscreen.style.setProperty('--color-primary', colors[0]);
              console.log(colors[0]);

              switch (lightOrDark(colors[0])) {
                case 'light':
                  colorPicker(wallpaper, { colors: 2, brightness: 0.25 }).then((colors1) => {
                    lockscreen.style.setProperty('--color-secondary', colors1[1]);
                    console.log(colors1[1]);
                  });
                  break;

                case 'dark':
                  colorPicker(wallpaper, { colors: 2, brightness: 1.75 }).then((colors1) => {
                    lockscreen.style.setProperty('--color-secondary', colors1[1]);
                    console.log(colors1[1]);
                  });
                  break;
      ...
      SettingsObserver.addObserver(WALLPAPER_KEY, this.observer);
    },
    ...
    stop: function() {
      ...
      SettingsObserver.removeObserver(WALLPAPER_KEY, this.observer);
      this._started = false;
    },
    ...
    _validate: function() {
      ...
      SettingsObserver.createLock().set(settings);
    },
