...
function Card(app, { disableScreenshots, stayInvisible } = {}) {
  ...
  var clearAllButton = document.getElementById('task-manager-clear-all-button');
  var clearAll = () => {
    clearAllButton.classList.add('clearing');
    el.classList.add('closing');

    setTimeout(() => {
      clearAllButton.classList.remove('clearing');
      el.classList.remove('closing');

      if (el.querySelector('.close-button')) {
        el.querySelector('.close-button').click();
      }
      clearAllButton.removeEventListener('click', clearAll, true);
    }, 300);
  };
  ...
  this.swipeAction = new SwipeToKillMotion(this.element, {
    setTranslateY: (y) => {
      this.translate({ y: y });
      var screenshot = this.element.querySelector('.screenshotView');

      var sy = -(y / ((window.innerHeight / 4) * 3)) * 90;
      var so = 1 + (y / ((window.innerHeight / 4) * 3));

      screenshot.style.transform = 'rotate3d(1, 0, 0, ' + sy + 'deg) translateY(' + -(sy * 0.5) + '%)';
      this.element.style.opacity = so;
    }
  });
}
