...
function AddListeners() {
  ...
  for (st = 0; st < gStorageTypes.length; st++) {
    ...
    storages = navigator.b2g.getDeviceStorages(storageType);
...
function UpdateStorage() {
  ...
  if (storageType == gStorageType) {
    ...
    // anything there.
    gDefaultStorage = navigator.b2g.getDeviceStorage(gStorageType);
    return;
  }
  ...
  gDefaultStorage = navigator.b2g.getDeviceStorage(gStorageType);
  gVolumeStorages = navigator.b2g.getDeviceStorages(gStorageType);
...
function ds_test() {
  ...
  var throws = false;
  try {
   storage = navigator.b2g.getDeviceStorage();
  } catch (e) { throws = true; }
  ...
  storage = navigator.b2g.getDeviceStorage('kilimanjaro');
  ...
  storage = navigator.b2g.getDeviceStorage('music');
  ok(storage, 'music - Should have getDeviceStorage');

  storage = navigator.b2g.getDeviceStorage('videos');
